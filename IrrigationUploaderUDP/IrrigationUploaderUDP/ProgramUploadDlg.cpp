...
IMPLEMENT_DYNAMIC(CProgramUploadDlg, CUploadDlg)

CProgramUploadDlg::CProgramUploadDlg(CWnd* pParent /*=NULL*/): CUploadDlg(IDD_DIALOG_PROGRAMS, pParent)
{
	m_nLastStation = 0;
...
void CProgramUploadDlg::EnableControls(const bool bEnable)
{
...
void CProgramUploadDlg::InitFields()
{
	...
	if (!m_strIPAddr.IsEmpty() && (m_nPort > 0))
	{
		for (nI = 0; nI < 8; nI++)
		{
			strStation.Format("Fetching current settings for station %d.", nI + 1);
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("radfrq", nI + 1, m_arrayStationSettings[nI].m_nRadioFreq);
			nProgress += 2;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("selfrq", nI + 1, m_arrayStationSettings[nI].m_nSelFreq);
			nProgress += 1;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("runfrq", nI + 1, m_arrayStationSettings[nI].m_nRunFreq);
			nProgress += 2;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("runtme", nI + 1, m_arrayStationSettings[nI].m_strRunTime);
			nProgress += 1;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("stme", nI + 1, m_arrayStationSettings[nI].m_strTimeStart);
			nProgress += 2;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("etme", nI + 1, m_arrayStationSettings[nI].m_strTimeEnd);
			nProgress += 1;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("sdte", nI + 1, m_arrayStationSettings[nI].m_strDateStart);
			nProgress += 2;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("edte", nI + 1, m_arrayStationSettings[nI].m_strDateEnd);
			nProgress += 1;
			dlg.SetProgress(nProgress, strStation);

			GetDataItem("desc", nI + 1, m_arrayStationSettings[nI].m_strDescription);
			nProgress += 1;
			dlg.SetProgress(nProgress, strStation);
		}
		dlg.Close();
...
void CProgramUploadDlg::EnableUploadButton()
{
	...
	m_buttonUpload.EnableWindow(bEnable && m_pSocket);
}
...
void CProgramUploadDlg::OnClickedButtonUpload()
{
	...
	if (!m_strIPAddr.IsEmpty() && (m_nPort > 0))
	{
		strReq = CString("program") + m_pSocket->GetDelim();
		for (nI = 0; nI < 8; nI++)
		{
			...
			strReq.Format("%s%d%s", strReq, nI + 1, m_pSocket->GetDelim());
			strReq.Format("%s%s%s", strReq, m_arrayStationSettings[nI].m_strDescription, m_pSocket->GetDelim());
			strReq.Format("%s%s%s", strReq, m_arrayStationSettings[nI].m_strDateStart, m_pSocket->GetDelim());
			strReq.Format("%s%s%s", strReq, m_arrayStationSettings[nI].m_strDateEnd, m_pSocket->GetDelim());
			strReq.Format("%s%d%s", strReq, m_arrayStationSettings[nI].m_nRadioFreq, m_pSocket->GetDelim());
			strReq.Format("%s%d%s", strReq, m_arrayStationSettings[nI].m_nSelFreq, m_pSocket->GetDelim());
			strReq.Format("%s%d%s", strReq, m_arrayStationSettings[nI].m_nRunFreq, m_pSocket->GetDelim());
			strReq.Format("%s%s%s", strReq, m_arrayStationSettings[nI].m_strRunTime, m_pSocket->GetDelim());
			strReq.Format("%s%s%s", strReq, m_arrayStationSettings[nI].m_strTimeStart, m_pSocket->GetDelim());
