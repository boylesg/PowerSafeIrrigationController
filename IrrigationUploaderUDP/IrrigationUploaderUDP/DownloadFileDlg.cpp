...
IMPLEMENT_DYNAMIC(CDownloadFileDialog, CUploadDlg)

CDownloadFileDialog::CDownloadFileDialog(CWnd* pParent /*=NULL*/): CUploadDlg(IDD_DIALOG_DOWNLOAD_FILE, pParent)
{
}
...
void CDownloadFileDialog::OnClickedButtonDownload()
{
	...
	if (!m_strIPAddr.IsEmpty() && (m_nPort > 0))
	{
		...
		strReq = CString("filesize") + m_pSocket->GetDelim() + strFilename;
		if (m_pSocket->Request(strReq, nFileSize, m_strIPAddr, m_nPort))
		{
			dlg.SetProgress(nProgress, "Downloading file '" + strFilename + "'");
			while (nFilePos < nFileSize)
			{
				strFilePos.Format("%d", nFilePos);
				strReq = CString("download") + m_pSocket->GetDelim() + strFilename + m_pSocket->GetDelim() + strFilePos;
