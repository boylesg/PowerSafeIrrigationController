...
IMPLEMENT_DYNAMIC(CFileUploadDlg, CUploadDlg)

CFileUploadDlg::CFileUploadDlg(CWnd* pParent /*=NULL*/): CUploadDlg(IDD_IRRIGATIONUPLOADERUDP_DIALOG, pParent)
{
...
void CFileUploadDlg::OnButtonUpload()
{
	...
	if (!m_strIPAddr.IsEmpty() && (m_nPort > 0))
	{
		...
		while (strContents.GetLength() > 0)
		{
			...
			strReq = CString("upload") + m_pSocket->GetDelim() + strUploadFilename + m_pSocket->GetDelim() + strByteCount + m_pSocket->GetDelim();
