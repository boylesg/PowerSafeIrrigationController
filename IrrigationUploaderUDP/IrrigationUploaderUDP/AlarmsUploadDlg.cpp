...
IMPLEMENT_DYNAMIC(CAlarmsUploadDlg, CUploadDlg)

CAlarmsUploadDlg::CAlarmsUploadDlg(CWnd* pParent /*=NULL*/): CUploadDlg(IDD_DIALOG_WALARMS, pParent)
{
	m_nLastStation = 0;
}
...
void CAlarmsUploadDlg::RestoreAlarmDetails(const int nStation)
{
	if ((nStation >= 0) && (nStation < 8))
	{
		...
		m_editDesc.SetWindowText(m_arrayAlarmDetails[nStation].m_strDescription);
...
void CAlarmsUploadDlg::EnableControls(const bool bEnable)
{
	...
	m_staticStatus.EnableWindow(bEnable);
	m_buttonUpload.EnableWindow(bEnable);
}

void CAlarmsUploadDlg::InitFields()
{
	...
	if (!m_strIPAddr.IsEmpty() && (m_nPort > 0))
	{
		for (nI = 0; nI < 8; nI++)
		{
			...
			GetDataItem("desc", nI + 1, m_arrayAlarmDetails[nI].m_strDescription);
			nProgress += 2;
			dlg.SetProgress(nProgress);

			GetDataItem("inst", nI + 1, m_arrayAlarmDetails[nI].m_bInstalled);
			nProgress += 2;
			dlg.SetProgress(nProgress);

			GetDataItem("auto", nI + 1, m_arrayAlarmDetails[nI].m_bAuto);
			nProgress += 2;
			dlg.SetProgress(nProgress);

			GetDataItem("dry", nI + 1, m_arrayAlarmDetails[nI].m_nDryTime);
			nProgress += 2;
			dlg.SetProgress(nProgress);

			GetDataItem("on", nI + 1, m_arrayAlarmDetails[nI].m_nOnTime);
			nProgress += 2;
			dlg.SetProgress(nProgress);

			GetDataItem("thresh", nI + 1, m_arrayAlarmDetails[nI].m_nThreshold);
			nProgress += 2;
			dlg.SetProgress(nProgress);
		}
		dlg.Close();
...
void CAlarmsUploadDlg::OnClickedButtonUpload()
{
	...
	if (ValidateFields(strResp) && !m_strIPAddr.IsEmpty() && (m_nPort > 0))
	{
		m_staticStatus.SetWindowTextA("Uploading alarm details...");
		if (!m_strIPAddr.IsEmpty() && (m_nPort > 0))
		{
			strReq = CString("alarms") + m_pSocket->GetDelim();
			for (int nI = 0; nI < 8; nI++)
			{
				...
				strReq.Format("%s%d%s", strReq, m_arrayAlarmDetails[nI].m_bInstalled, m_pSocket->GetDelim());
				strReq.Format("%s%d%s", strReq, m_arrayAlarmDetails[nI].m_bAuto, m_pSocket->GetDelim());
				strReq.Format("%s%d%s", strReq, m_arrayAlarmDetails[nI].m_nDryTime, m_pSocket->GetDelim());
				strReq.Format("%s%d%s", strReq, m_arrayAlarmDetails[nI].m_nThreshold, m_pSocket->GetDelim());
